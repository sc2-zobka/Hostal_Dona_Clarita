    ...
    with connection.cursor() as cursor:
        try:
            ...
            if infoCliente[0] and infoCliente[1] and infoCliente[2] and infoCliente[3]:
                ...
                #Si Posee Datos Busca Servicios Adioscionales
                else:
                    cursor.execute(
                        ...
                        if int(nhuesped) >= 1: # VALIDA QUE HUÉSPED SEA AL MENOS 1
                            with connection.cursor() as cursor:
                                try:
                                    ...
                                    cursor.execute("""
                                    select h.numero_habitacion, h.id, h.descripcion, h.tipo_habitacion_id, th.descripcion, th.capacidad
                                    from app_habitacion h
                                    join app_tipohabitacion th ON h.tipo_habitacion_id = th.id
                                    where h.numero_habitacion not in (select h1.numero_habitacion
                                                from app_habitacion h1
                                                inner join app_reserva r
                                                on h1.id = r.habitacion_id
                                                inner join app_detalleordencompra doc
                                                on r.detalle_orden_compra_id = doc.id
                                                where %s between doc.inicio_estadia and doc.final_estadia)
                                    """, [fdesde])
                                    nrohabitaciones = cursor.fetchall()
                                    if not nrohabitaciones:# VALIDA HABITACIONES DISPONIBLES EN RANGO DE FECHA
                                        messages.error(request, 'No existe Disponibilidad para la cantidad de Huéspedes en el Periodo de Fecha Indicado')
                                        ...
                                        with connection.cursor() as cursor:
                                            try:
                                                ...
                                                if capacidad == None:
                                                    ...
                                                    #
                                                    cursor.execute(
                                                        "SELECT ID FROM APP_ORDENCOMPRA WHERE FECHA_EMISION = %s", [fecha_emision])
                                                    ...
                                                    for index in range(len(empleados_chk)):
                                                        ...
                                                        #
                                                        cursor.execute("SELECT ID FROM APP_HABITACION WHERE NUMERO_HABITACION = %s",
                                                        ...
                                                        #
                                                        cursor.execute("SELECT ID FROM APP_TIPOCOMEDOR WHERE ID = %s",
